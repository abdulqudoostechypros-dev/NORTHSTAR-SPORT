    ...
    <footer className="mt-20 bg-navy text-white">
      <div className="wrap py-12">
        <div className="grid gap-10 border-b border-white/15 pb-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
          ...
          <div><p className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-white/40">Explore</p><div className="flex flex-col gap-3 text-sm text-white/70"><Link href="/about">About</Link><Link href="/services">Services</Link><Link href="/portfolio">Portfolio</Link></div></div>
          <div><p className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-white/40">Connect</p><div className="flex flex-col gap-3 text-sm text-white/70"><Link href="/why-choose-us">Why Northstar</Link><Link href="/faq">FAQ</Link><Link href="/contact">Contact</Link></div></div>
          <div><p className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-white/40">Connect</p><p className="text-sm leading-relaxed text-white/70">Use the contact form<br />to start a conversation.</p><Link href="/contact" className="mt-3 inline-flex text-sm font-semibold text-coral">Contact Northstar <span className="ml-1">↗</span></Link></div>
        </div>
        <div className="flex flex-col justify-between gap-4 pt-7 text-xs text-white/40 sm:flex-row">
          ...
          <div className="flex flex-wrap gap-5"><Link href="/privacy" className="transition hover:text-white">Privacy Policy</Link><Link href="/cookies" className="transition hover:text-white">Cookie Policy</Link><Link href="/terms" className="transition hover:text-white">Terms</Link><span>Independent by design.</span></div>
        </div>
      </div>
    </footer>
  );
