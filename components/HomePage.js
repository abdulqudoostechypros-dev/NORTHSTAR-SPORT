...
function LinkButton({ children, href = "/contact", secondary = false }) {
  return <Link href={href} className={`inline-flex items-center justify-center gap-2 rounded-full px-6 py-3.5 text-sm font-bold transition ${secondary ? "border border-ink/20 text-ink hover:border-coral hover:text-coral" : "bg-coral text-white hover:bg-white hover:text-ink"}`}>{children}<ArrowUpRight className="h-4 w-4" /></Link>;
}

function SectionIntro({ eyebrow, title, description, href, linkLabel }) {
  return <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div className="max-w-2xl"><p className="eyebrow">{eyebrow}</p><h2 className="display-title mt-4">{title}</h2>{description && <p className="mt-6 max-w-xl text-base leading-relaxed text-ink-soft">{description}</p>}</div>{href && <Link href={href} className="inline-flex shrink-0 items-center gap-2 text-sm font-bold text-coral transition hover:text-ink">{linkLabel}<ArrowUpRight className="h-4 w-4" /></Link>}</div>;
}
...
export default function HomePage() {
  return (
    <main className="min-h-screen overflow-hidden">
      ...
      <section className="bg-white py-20 lg:py-28" id="faq"><div className="wrap grid gap-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow">FAQ</p><h2 className="display-title mt-4">Useful answers, up front.</h2><p className="mt-6 text-base leading-relaxed text-ink-soft">Still have a question? We are happy to make the next step specific.</p><Link href="/faq" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-coral">View all FAQs <ArrowRight className="h-4 w-4" /></Link></div><div className="divide-y divide-line border-y border-line">{faqs.map(([question, answer], index) => <details key={question} className="group py-5" open={index === 0}><summary className="flex cursor-pointer list-none items-center justify-between gap-5 font-display text-lg font-bold">{question}<Plus className="h-5 w-5 shrink-0 group-open:hidden" /><Minus className="hidden h-5 w-5 shrink-0 group-open:block" /></summary><div className="faq-answer"><p className="max-w-2xl pt-4 text-base leading-7 text-ink-soft">{answer}</p></div></details>)}</div></div></section>

      <section className="relative overflow-hidden bg-coral py-20 text-white lg:py-28"><div className="absolute -right-24 -top-36 h-[520px] w-[520px] rounded-full border border-white/20" /><div className="wrap relative flex flex-col justify-between gap-10 lg:flex-row lg:items-end"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-white/70">Ready to move?</p><h2 className="mt-5 font-display text-5xl font-bold leading-[.98] tracking-[-.05em] sm:text-7xl">Bring us the question behind the brief.</h2><p className="mt-7 max-w-xl text-lg leading-relaxed text-white/80">Tell us what you are building, changing, or trying to understand. We will come back with a useful next step.</p></div><div className="flex shrink-0 flex-wrap gap-3"><Link href="/contact" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-bold text-ink transition hover:bg-ink hover:text-white">Start a conversation <ArrowUpRight className="h-4 w-4" /></Link><Link href="/contact" className="inline-flex items-center gap-2 rounded-full border border-white/40 px-6 py-3.5 text-sm font-bold text-white transition hover:bg-white/10">View contact options</Link></div></div></section>
